...
label_files = sorted(
    glob(
        "G:\\elwha_ortho_segmentation\\seg_zoo_model_datasets\\elwha_aerial\\v8_june2023_all\\labels_all\\*.tif"
    )
)
...
for k in label_files:
    ...
    if np.all(np.unique(lab) == np.array([0, 1, 2, 3, 4])):
        ...
        shutil.copyfile(
            k.replace(orig_label_dir, orig_image_dir),
            k.replace(orig_label_dir, target_dir_images),
        )
