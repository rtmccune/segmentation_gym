...
root.filename = askopenfilename(
    initialdir="./",
    title="Select text file listing model output folders to process",
...
root.filename = askopenfilename(
    ...
    title="Select text file listing wld folders to process",
    filetypes=(("txt file", "*.txt"), ("all files", "*.*")),
)
folderlist = root.filename
print(folderlist)
...
root.filename = askopenfilename(
    initialdir=folders[0],
    title="Select text file listing xml folders to process",
    filetypes=(("txt file", "*.txt"), ("all files", "*.*")),
)
folderlist = root.filename
print(folderlist)
...
root.filename = askopenfilename(
    initialdir=folders[0],
    title="Select text file listing output folders",
    filetypes=(("txt file", "*.txt"), ("all files", "*.*")),
)
folderlist = root.filename
print(folderlist)
...
for output, xml, wld, folder in zip(outputs, xmls, wlds, folders):
    ...
    os.system(
        "for f in "
        + output
        + '/*.png; do convert $f -quality 100 "${f%_predseg.png}.jpg"; done'
    )
    ...
    os.system(
        "for k in "
        + output
        + '/*.jpg; do gdal_translate -of GTiff -b 1 $k "${k%jpg}tif"; done'
    )
    ...
    os.system(
        "gdal_merge.py -o "
        + output
        + '/mosaic.tif -of GTiff -co "COMPRESS=JPEG" -co "TILED=YES"  '
        + output
        + "/*.tif"
    )
